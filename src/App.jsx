...
// Per-tab storage keys (v2 — bumped to invalidate old caches missing zh names)
const TAB_KEYS = {
  recent: {
    obs: "taiwan_birds_recent_obs_cache_v2",
    days: "taiwan_birds_recent_days_setting",
    time: "taiwan_birds_recent_last_fetch_time_v2",
  },
  notable: {
    obs: "taiwan_birds_notable_obs_cache_v2",
    days: "taiwan_birds_notable_days_setting",
    time: "taiwan_birds_notable_last_fetch_time_v2",
  },
};
...
const STORAGE_TAXONOMY_KEY = "taiwan_birds_taxonomy_cache_v3";
const STORAGE_TAXONOMY_TIME_KEY = "taiwan_birds_taxonomy_time_v3";
...
export default function App() {
  ...
  const fetchObservations = useCallback(async (tab, selectedDays, activeKey = apiKey) => {
    ...
    try {
      ...
      if (notable) {
        ...
        // Fetch EN for selected regions sequentially (1.5s delay)
        dataEn = [];
        for (const r of regionsToFetch) {
          const url = `https://api.ebird.org/v2/data/obs/${r}/recent?back=${selectedDays}&detail=simple&includeProvisional=true`;
          const result = await fetchWithRetry(url);
          dataEn = dataEn.concat(result);
          await new Promise(r => setTimeout(r, 1500));
        }
        // No ZH per-region fetch — use taxonomy cache for all Chinese names
        dataZh = [];
      }
  ...
  // Auto-load: try static JSON first (instant), fall back to localStorage, then API
  useEffect(() => {
    ...
    const run = async () => {
      ...
      // 1. Try static JSON (pre-generated by cron, shared cache)
      ...
      // 2. Try localStorage cache (same-day)
      ...
      // 3. Live API fetch as last resort
